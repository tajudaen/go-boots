...
func TestCreateRepoInvalidSuccessResponse(t *testing.T) {
	...
	assert.NotNil(t, err)
	assert.EqualValues(t, http.StatusInternalServerError, err.StatusCode)
	assert.EqualValues(t, "error when trying to unmarshal github create repo", err.Message)
}
...
func TestCreateRepoNoError(t *testing.T) {
	...
	restclient.AddMockup(restclient.Mock{
		...
		Response: &http.Response{
			StatusCode: http.StatusCreated,
			Body:       ioutil.NopCloser(strings.NewReader(`{"id": 1234, "name": "golang-tut", "full_name": "john doe"}`)),
		},
	})
	...
	assert.EqualValues(t, "golang-tut", response.Name)
	assert.EqualValues(t, "john doe", response.FullName)
}
